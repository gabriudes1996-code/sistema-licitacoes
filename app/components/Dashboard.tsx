...
import { BadgeCheck, BadgePercent, Banknote, CircleDollarSign, Coins, FileText, Hourglass, ReceiptText, Target, TrendingUp, WalletCards } from "lucide-react";
...
const CUSTOS_EMPRESA_KEY = "licitapro_custos_empresa";
...
export default function Dashboard() {
  ...
  const carregar = () => {
    try {
      ...
      let despesas:CustoEmpresa[]=[];try{despesas=JSON.parse(localStorage.getItem(CUSTOS_EMPRESA_KEY)||"[]")}catch{}
      setCustosEmpresa(Array.isArray(despesas)?despesas.reduce((s,c)=>s+n(c.valor),0):0);
      setLicitacoes(Array.isArray(lista) ? lista : []); setItens(mapa); setRecebido(totalRecebido); setSolicitado(totalSolicitado);
    } catch { setLicitacoes([]); setItens({}); setRecebido(0); setSolicitado(0); setCustosEmpresa(0); }
  };

  useEffect(() => {
    carregar(); const timer=setInterval(carregar,1000); window.addEventListener("storage",carregar); window.addEventListener("licitapro-custos-updated",carregar);
    return()=>{clearInterval(timer);window.removeEventListener("storage",carregar);window.removeEventListener("licitapro-custos-updated",carregar)};
  }, []);

  const resumo = useMemo(() => {
    let andamento=0,concluidas=0,perdidas=0,custoGanhos=0,valorGanhos=0;
    licitacoes.forEach((l)=>{const lista=itens[l.id]||[];const resultados=lista.map(i=>i.resultado||"em_disputa");const todosFinalizados=lista.length>0&&resultados.every(r=>r==="ganhou"||r==="perdeu");const todosPerdidos=lista.length>0&&resultados.every(r=>r==="perdeu");if(todosFinalizados&&!todosPerdidos)concluidas++;else if(todosPerdidos)perdidas++;else andamento++;lista.forEach(i=>{if(i.resultado!=="ganhou")return;const qtd=n(i.quantidade);custoGanhos+=qtd*n(i.custo)+n(i.frete);valorGanhos+=qtd*n(i.lanceAtual)})});
    const lucroPotencial=valorGanhos-custoGanhos;const margem=custoGanhos>0?(lucroPotencial/custoGanhos)*100:0;const taxaSucesso=(concluidas+perdidas)>0?(concluidas/(concluidas+perdidas))*100:0;const saldoContrato=Math.max(0,valorGanhos-solicitado);const pendentePagamento=Math.max(0,solicitado-recebido);const percentualRecebido=solicitado>0?(recebido/solicitado)*100:0;const dinheiroCaixa=recebido-custosEmpresa-custoGanhos;
    return{andamento,concluidas,perdidas,custoGanhos,valorGanhos,lucroPotencial,margem,taxaSucesso,saldoContrato,pendentePagamento,percentualRecebido,dinheiroCaixa};
  },[licitacoes,itens,recebido,solicitado,custosEmpresa]);

  return <section className="neoDashboard">
    <div className="neoIntro"><div><span>VISÃO EXECUTIVA</span><h2>Dashboard</h2><p>Licitações, contratos e resultado financeiro em uma visão organizada.</p></div><button onClick={carregar} type="button">Atualizar dados</button></div>
    <div className="neoTopGrid neoTopGridSix">
      <RingCard titulo="Licitações em andamento" valor={String(resumo.andamento)} detalhe="Disputas ainda abertas" percentual={licitacoes.length?(resumo.andamento/licitacoes.length)*100:0} classe="cyan" icon={<FileText size={17}/>} />
    ...
    <div className="neoCashSummary">
      <TopMoney icon={<Banknote size={20}/>} classe="lime" label="Dinheiro em caixa" value={brl(resumo.dinheiroCaixa)} detail="Recebido − contas da empresa − custo dos ganhos" />
      <TopMoney icon={<ReceiptText size={20}/>} classe="pink" label="Contas da empresa" value={brl(custosEmpresa)} detail="Aluguel, energia, cartão, internet e demais despesas" />
    </div>
    <div className="neoGroupedDashboard neoContractVisual"><div className="neoSectionHeader"><span>EXECUÇÃO DOS CONTRATOS</span><small>Do pedido da prefeitura até o recebimento</small></div><div className="neoContractGrid">
...
function Money({icon,classe,label,value,detail}:{icon:ReactNode;classe:string;label:string;value:string;detail:string}){return <div className={`neoMoneyRow grouped flow ${classe}`}><div className={`neoMoneyIcon ${classe}`}>{icon}</div><div><span>{label}</span><strong>{value}</strong><small>{detail}</small></div></div>}
function InfoCard({icon,titulo,valor,detalhe,classe}:{icon:ReactNode;titulo:string;valor:string;detalhe:string;classe:string}){return <div className="neoMiniStatus neoInfoCard grouped"><div className={`neoMoneyIcon ${classe}`}>{icon}</div><div><span>{titulo}</span><strong>{valor}</strong><small>{detalhe}</small></div></div>}
